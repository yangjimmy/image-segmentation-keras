...
def preprocess(scan_dirs):
    ...
    for scan_dir in scan_dirs:
        ...
        #         print('E:\YKL\Thorlabs VSCAN Labeling\scan30\{}'.format(img_fl))
                ...
                # write to file
                cv2.imwrite('dataset2\\images\\{}_{}_{}'.format(scan_dir,image_idx,img_fl),resized_denoised_img)
                
                # X.append(resized_denoised_img)

                msk = cv2.imread('{}\\Label_{}_{}'.format(label_dir,image_idx,img_fl.split('.')[0]+'.png'), cv2.IMREAD_GRAYSCALE)
                resized_msk = np.clip(cv2.resize(msk,(384, 352), interpolation = cv2.INTER_NEAREST),0,4)
                ...
                #     raise ValueError
                cv2.imwrite('dataset2\\labels\\{}_{}_{}'.format(scan_dir,image_idx,img_fl),resized_msk)
